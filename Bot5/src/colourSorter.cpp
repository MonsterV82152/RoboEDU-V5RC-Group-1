...
class ColourSorter {
            ...
            while (true) {
                ...
                if (distanceSensor->get() < DISTANCE_THRESHOLD) {
                    if (firstDetection) {
                        ...
                        if ((rings[0]-1) == !team && shouldSort) hook->setOverwriteSpeed(-10, reverseCountdown,true);
                        removeFirstRing();
                        master.rumble(".");
    ...
    public:
        ColourSorter(Hook *hook, Intake *intake, pros::Optical *colourSensor, pros::Distance *distanceSensor, int delay)
            : hook(hook), intake(intake), colourSensor(colourSensor), distanceSensor(distanceSensor), sorterTask(nullptr), shouldSort(false), delayTime(delay), noRing(true), firstDetection(true), reverseCountdown(5), lastColor(0) {}
