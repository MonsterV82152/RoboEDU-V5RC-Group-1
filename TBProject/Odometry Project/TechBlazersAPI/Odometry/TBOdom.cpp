...
void TankDR::moveToPoint(float x, float y, float timeout, bool forwards = true, float maxSpeed = 127, float minSpeed = 0) {
    ...
    movement.create([&]() {
        ...
        while ((abs(xdif) >= 0.1 || abs(ydif) >= 0.1) && !exit) {
            ...
            if (leftresult > maxSpeed) {leftresult = maxSpeed;} else if (leftresult < minSpeed) {leftresult = minSpeed;}
            if (rightresult > maxSpeed) {rightresult = maxSpeed;} else if (rightresult < minSpeed) {rightresult = minSpeed;}
